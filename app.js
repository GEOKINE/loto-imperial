...
async function initializeApp() {
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            // If we are in profile or admin, show main sections again before scrolling
            document.querySelectorAll('.section').forEach(s => {
                if (!['profile', 'admin'].includes(s.id)) {
            ...
            try {
                ...
                feedback.innerHTML = `
                    <div style="text-align:center; color: var(--primary-highlight); margin-top:20px;">
                        ...
                        <div class="res-code-badge" style="display:inline-block; margin: 15px 0; font-size: 1.5rem; padding: 10px 20px;">
                            ${specialCode}
                        </div>
        ...
        try {
            if (currentAuthMode === 'login') {
                ...
                const { data, error } = await supabase.auth.signUp({
                    ...
                    password,
                    options: {
                        data: { full_name: authFullName.value }
                    }
                });
                if (error) throw error;
    ...
    async function updateAuthUI(user) {
        if (user) {
            ...
            if (profile?.role === 'admin') {
                accountLabel.innerText = 'Panel Admin';
            }
        } else {
    ...
    async function loadUserProfile() {
        ...
        const { data: res, error } = await supabase.from('reservations').select('*').eq('user_id', session.user.id).order('date', { ascending: false });
    ...
    async function loadAdminPanel() {
        const { data: res, error } = await supabase.from('reservations').select('*, profiles(full_name)').eq('status', 'pendiente').order('date', { ascending: true });
    ...
    document.getElementById('btnSearchCode').addEventListener('click', async () => {
        ...
        if (error || !data) {
            ...
            showNotification(`Encontrado: ${data.profiles?.full_name}`, 'success');
        }
    });
}
